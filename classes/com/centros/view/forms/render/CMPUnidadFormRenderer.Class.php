...
class CMPUnidadFormRenderer extends DefaultFormRenderer {
	...
	protected function renderFieldset(CMPForm $form, XTemplate $xtpl){
		$xtpl->assign("sigla_requerida", CYT_MSG_UNIDAD_SIGLA_REQUIRED);
		foreach ($form->getFieldsets() as $fieldset) {
			...
			$fieldDenominacion = $fields['sigla'];		
			$input = $fieldDenominacion->getInput();
			$label = $fieldDenominacion->getLabel();	
			$this->renderLabel( $label, $input, $xtpl );
			$this->renderInput( $input, $xtpl );
			...
			$xtpl->parse("main.fieldset.column.sigla");
			//print_r($input);
			$xtpl->assign("sigla", $input->getInputValue());
			...
			$fieldEspecialidad = $fields['dt_disposicion'];		
			$input = $fieldEspecialidad->getInput();
			$label = $fieldEspecialidad->getLabel();	
			$this->renderLabel( $label, $input, $xtpl );
			$this->renderInput( $input, $xtpl );
			...
			$xtpl->parse("main.fieldset.column.dt_disposicion");
			
			$fieldEspecialidad = $fields['nu_disposicion'];		
			$input = $fieldEspecialidad->getInput();
			$label = $fieldEspecialidad->getLabel();	
			$this->renderLabel( $label, $input, $xtpl );
			$this->renderInput( $input, $xtpl );
			...
			$xtpl->parse("main.fieldset.column.nu_disposicion");
			...
			$xtpl->assign("value", CYT_LBL_UNIDAD_MEMORIAS );
			$xtpl->assign("required", "*" );
			$xtpl->parse("main.memorias.label");
			$xtpl->assign("actionFile", "doAction?action=add_file_session" );
			$xtpl->parse("main.memorias.input");
			$xtpl->assign("display", 'block');
			$hiddens = $form->getHiddens();
			$hiddenDs_curriculum = $hiddens['memorias'];	
				
			if ($hiddenDs_curriculum->getInputValue()) {
				$xtpl->assign("memorias_cargado", '<span style="color:#009900; font-weight:bold">'.CYT_MSG_FILE_UPLOAD_EXITO.$hiddenDs_curriculum->getInputValue().'</span>');
			}
			$xtpl->parse("main.memorias");
			...
			$xtpl->assign("value", CYT_LBL_UNIDAD_REGLAMENTO );
			$xtpl->assign("required", "*" );
			$xtpl->parse("main.reglamento.label");
			$xtpl->assign("actionFile", "doAction?action=add_file_session" );
			$xtpl->parse("main.reglamento.input");
			$xtpl->assign("display", 'block');
			$hiddens = $form->getHiddens();
			$hiddenDs_curriculum = $hiddens['reglamento'];	
				
			if ($hiddenDs_curriculum->getInputValue()) {
				$xtpl->assign("reglamento_cargado", '<span style="color:#009900; font-weight:bold">'.CYT_MSG_FILE_UPLOAD_EXITO.$hiddenDs_curriculum->getInputValue().'</span>');
			}
			$xtpl->parse("main.reglamento");
